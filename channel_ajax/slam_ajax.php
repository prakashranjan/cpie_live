...
include("../vcommon.php");
 ...
 $jql="SELECT s_id FROM channel_slam WHERE username='$usern' AND topic_id='$tid' AND chat_id='$chat_id'";
 $dar=mysql_query($jql);
 if ($par=mysql_fetch_row($dar))
 { $tql="DELETE from channel_slam where username='$usern' and topic_id='$tid' and chat_id='$chat_id'";
	$tar=mysql_query($tql);
	
	$mql5="update channel_post set eye= eye-1 where  image_id='$chat_id'";
				$kar5=mysql_query($mql5);
				
				$qry="SELECT eye FROM channel_post WHERE image_id='$chat_id'";
...
{
 $sql="INSERT INTO channel_slam (username,topic_id,chat_id) VALUES ('$usern','$tid','$chat_id')";
			     $toname=mysql_query($sql);
				
				

				$mql56="update channel_post set eye= eye+1 where image_id='$chat_id'";
				$kar56=mysql_query($mql56);
				
				$qry89="SELECT eye FROM channel_post WHERE image_id='$chat_id'";
