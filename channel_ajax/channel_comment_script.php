					...
					if($usr!=$rowori[2] || $rowori[7]==1)
                        ...
                        echo'<li class="left clearfix"><span class="chat-img pull-left">
                            ...
                            <div class="chat-body clearfix">
                                ...
                                <p class="text-left text-info">
                                 ...
                                 <div class="text-right">';  $jql="SELECT s_id FROM channel_comment_slam WHERE username='$usern' AND comment_id='$rowori[0]'";
                                      $dar=mysql_query($jql);
                                      if ($tar=mysql_fetch_row($dar))
									  echo'<button id="'.$rowori[0].'comm_slam_id" onclick="slam_comment(this.id)" class="comment_slam text-success fa"><i class="fa fa-thumbs-o-up text-success"  aria-hidden="true"></i> '.$rowori[4].'</button>';
								  else
									  echo'<button id="'.$rowori[0].'comm_slam_id" onclick="slam_comment(this.id)" class="comment_slam text-muted fa"><i class="fa fa-thumbs-o-up text-muted" aria-hidden="true"></i> +'.$rowori[4].'</button>';
                                      echo'</div>
                            </div>
                                        </li>';}
                                        else{
						...
						$rowori[2]="*******";
                            ...
                            <div class="chat-body clearfix">
                                ...
                                <div class="text-left">';  $jql="SELECT s_id FROM channel_comment_slam WHERE username='$usern' AND comment_id='$rowori[0]'";
                                      $dar=mysql_query($jql);
                                      if ($tar=mysql_fetch_row($dar))
									  echo'<button id="'.$rowori[0].'comm_slam_id" onclick="slam_comment(this.id)" class="comment_slam text-success fa"><i class="fa fa-thumbs-o-up text-success"  aria-hidden="true"></i> '.$rowori[4].'</button>';
								  else
