...
if($type_id==""){exit();}
                 ...
                 $qry="update type set views=views+1 where branch_id='$type_id' ";
                 $res=mysql_query($qry);
                 $mqry="select type from type where branch_id='$type_id' ";
                 $mres=mysql_query($mqry);
                 $mrow=mysql_fetch_row($mres);
                 echo'
   <div class=" khatoono  "> " '.str_replace("#","",$mrow[0]).' " </div>
    ...
    <br>';
                 ...
                 $kqry="SELECT topic_id,category,cdesc,wall,private,topic_mode FROM topic WHERE type_id='$type_id' ORDER BY views DESC";
$y=-1;
$j=0;
//$p=0;
...
<div class="thumbnail img-rounded responsive" style="background-color:'.$block.'; border:none;  max-height: 120px;
     height:120px;" > <!--  thumbnail 1  -->
          
           <div class="caption " style="padding-bottom:0px;background:url('.$source.') center no-repeat;"> <!-- caption -->
     <blockquote class="img-rounded "style="background-color:'.$blockn.'; padding-left:2px;padding-right:2px; border:none; margin:0 0 0; height:100px; max-height:100px;overflow:hidden;"><p style="margin: 0 0 0;font-size:0.9em;background-color:rgba(255,255,255,1.0);color:'.$colpr.'; font-weight:bolder;     box-shadow: 2px 2px 15px #fff !important;"class="ubuntu">@'.$krow[1].''.$lock.'</p>
            <footer class="'.$privfoot.'"style="font-style:italic;font-size:0.64em;" >'.$info.'</footer></blockquote>
				...
				if(!$krow[1]&& $y==-1) 
{echo'<img height="15%" width="15%"src="images/sorry.png"></img><br /><h2> Add Channels!! </h2>';}				
 ?>